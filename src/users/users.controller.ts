import { Controller, Get, Param, Patch, Post, Body, UsePipes, ValidationPipe, Request, Delete, UseGuards } from '@nestjs/common';
import { ApiTags, ApiOkResponse, ApiInternalServerErrorResponse, ApiNotFoundResponse, ApiParam, ApiBody, ApiCreatedResponse, ApiBadRequestResponse,
...
export class UsersController {
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth()
    @ApiParam({name: 'searchText', description: 'Part of name, lastname or email'})
    @ApiOkResponse({description: 'Some entity found'})
    @ApiInternalServerErrorResponse({description: 'Internal server error performing request'})
    ...
    @ApiNotFoundResponse({description: 'Entity created, but failed to read it'})
    async insert(
        @Body() entity: UserDto,
        @Request() req,
    ): Promise<User | undefined> {
    ...
    @ApiNotFoundResponse({description: 'Entity not found | Entity updated, but failed reading it back'})
